...
def get_driver_count():
    ...
    if request.method == 'POST'or 'GET':
        ...
        elif selected_option == 'driver_and_date_form':
            return redirect(url_for('driver_and_date_form'))
        elif selected_option == 'custom_search_form':
            return redirect(url_for('custom_search_form'))
...
@app.route('/driver_and_date_form', methods=['GET', 'POST'])
def driver_and_date_form():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        # Define the SQL query to join DRIVER and RIDE tables based on DRIVER_ID and DATE_OF_RIDE
        query = text("""
            SELECT *
            FROM DRIVER
            JOIN RIDE ON DRIVER.DRIVER_ID = RIDE.DRIVER_ID
            WHERE DRIVER.DRIVER_ID = :driver_id AND RIDE.DATE_OF_RIDE = :ride_date
        """)

        # Execute the query with user input as parameters
        params = {'driver_id': driver_id, 'ride_date': ride_date}
        results = db.session.execute(query, params).fetchall()

        # Render the results in a template
        return render_template('driver_and_date_result.html', results=results)

    return render_template('driver_and_date_form.html')

from sqlalchemy import text

@app.route('/custom_search_form', methods=['POST','GET'])
def custom_search_form():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        date_of_ride = request.form.get('date_of_ride')
        time_of_ride = request.form.get('time_of_ride')
        ...
        if ride_id:
            base_query += " AND RIDE_ID = :ride_id"
        if start_location:
            base_query += " AND START_LOCATION = :start_location"
        if end_location:
            base_query += " AND END_LOCATION = :end_location"
        if fare:
            base_query += " AND FARE = :fare"
        if date_of_ride:
            base_query += " AND DATE(DATE_OF_RIDE) = :date_of_ride"
        if time_of_ride:
            base_query += " AND TIME_OF_RIDE = :time_of_ride"
        if customer_id:
            base_query += " AND CUSTOMER_ID = :customer_id"
        if driver_id:
        ...
        # Render the results in a template
        return render_template('custom_search_result.html', results=results)

    return render_template('custom_search_form.html')
...
def join_query():
    ...
    # Display results 
    return render_template('join_query.html', result=result)
